...
function LoginForm() {
  ...
  /*
   * Auth Check
   * We use useAuth to check if the user is already logged in.
   * If they are, we redirect them to home (or next param).
   */
  const { user, isLoading } = useAuth(false); // false = do NOT redirect if unauthenticated (we are on login page)
